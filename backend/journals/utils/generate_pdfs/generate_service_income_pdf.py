...
class GenerateServiceIncomePDF:
    ...
    def create_pdf(self):
        ...
        if 'Service' in self.title:
            self.create_service_income()
    ...
    def create_service_income(self):
        
        self.add_service_income_sub_header()
        self.add_service_income_items()
        self.add_service_income_sub_footer()

    def add_service_income_sub_footer(self):
        ...
        if invoice:
            ...
            service_income_data.extend(invoice_data)
        else:
            more_data.append(['Amount Paid', items_data.get('cash')])

        
        service_income_data.extend(more_data)
        
        service_income_data.append(['Total', f"{self.organisation.currency} {items_data.get('total_amount')}"])

        self.add_payment_data(service_income_data)
        
       


    def add_service_income_items(self):
        service_income_entries = self.data.get("service_income_entries")
        service_income_entries_list = []

        for entry in service_income_entries:
            total_service_income_price =  float(entry.get("price")) *  entry.get("quantity")
            entry_list = [entry.get("service_name"), entry.get("quantity"), entry.get("price"), total_service_income_price]
            service_income_entries_list.append(entry_list)

        service_income_entries_data = [
            ["Service", "Quantity", "Price", "Total"]
        ]
        service_income_entries_data.extend(service_income_entries_list)

        self.create_table(service_income_entries_data)
    
    
    def capitalize(self, name):
        return name.title().replace('_', ' ')

    def add_service_income_sub_header(self):
        service_income_data = [
            ['Service Income #', self.data.get("serial_number"), 'Date', self.data.get("date")],
        ...
        self.add_info(service_income_data)
