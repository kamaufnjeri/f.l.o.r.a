...
import SupplierTable from "@/components/dashboard/suppliers/SuppliersTable";
...
export default async function SuppliersPage({
  ...
  const response = await getSuppliers(organisationId, {
    search,
    name,
    page,
  });
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-4 p-4">
      ...
      {/* EMPTY STATE */}
      {suppliers.length === 0 ? (
        <NoItem title="Suppliers" modalName="supplier"/>
      ) : (
        <>
