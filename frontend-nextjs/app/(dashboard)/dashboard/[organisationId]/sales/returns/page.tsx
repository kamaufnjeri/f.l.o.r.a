...
import { getSalesReturns } from "@/app/actions/returns-actions";
...
export default async function SalesReturnsPage({
  ...
  const response = await getSalesReturns(organisationId, {
    search,
    date,
    sort_by,
    page,
  ...
  const salesReturnsData = response.salesReturns || [];

  return (
    <div className="w-full max-w-7xl mx-auto space-y-4 px-4">
      {/* FILTERS */}
      <FiltersSection title='Sales Returns' goToUrl={'sales/returns'} filters={{ search, sort_by, date, page }} organisationId={organisationId} />

      {/* EMPTY STATE */}
      {salesReturnsData.length === 0 ? (
