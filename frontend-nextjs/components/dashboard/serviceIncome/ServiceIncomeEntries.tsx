...
type Props = {
  ...
  removeServiceIncomeEntry: (index: number) => void;
  disabled?: boolean;
  isDirty?: boolean;
  currency: string;
};

export default function ServiceIncomeEntries({
  ...
  return (
    <div className="bg-white rounded-3xl border shadow-sm p-6 space-y-6">
      ...
      <div className="space-y-3">
        {serviceIncomeEntries.map((item, index) => (
          <div
            key={index}
          ...
          >
            ...
            <div className="md:col-span-5">
              <SelectField
                ...
                options={services}
                onChange={(val) =>
                  updateServiceIncomeEntry(index, "service", val as string)
                }
                disabled={disabled}
              />
            </div>
            ...
            <div className="md:col-span-2 flex md:justify-end justify-start">
              {(!disabled && serviceIncomeEntries.length > 1) && (
                <button
                  type="button"
                  onClick={() => removeServiceIncomeEntry(index)}
