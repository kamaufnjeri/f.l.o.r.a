...
export default function ServiceIncomesTable({
  serviceIncomes,
  totals,
}: {
  serviceIncomes: SalesOverview[];
  totals: SalesTotal
}) {
  return (
    ...
    <table className="w-full min-w-[900px] text-sm">
      ...
      <thead className="sticky top-0 z-10 bg-gray-50 text-gray-600">
        <tr className="border-b">
          <th className="p-3 text-left font-medium">ServiceIncome #</th>
      ...
      <tbody className="divide-y">
        {serviceIncomes?.map((serviceIncome) => (
         
            <tr
              key={serviceIncome.id}
            ...
            >
              ...
              <td className="p-3 flex flex-col items-start">
                <div className="flex flex-wrap gap-1">
                  {serviceIncome.details.items.map((item, index) => (
                    <span
                      key={index}
              ...
              <td className="bg-gray-50/40 text-right pr-3">
                <Link
                  href={`service-income/${serviceIncome.id}`}
                  className="
                    inline-flex
                    items-center
                    gap-2
