...
const validationSchema = Yup.object({
    ...
    return_entries: Yup.array()
        .of(
            Yup.object().shape({
                sales_entry: Yup.string().required('Account is required'),
...
const SalesReturnModal = ({ openModal, setOpenModal, title, sale, onSalesReturn }) => {
            ...
            >
                <Formik
                    ...
                    onSubmit={async (values, { resetForm }) => {
                        ...
                        const response = await postRequest(values, `${orgId}/sales_returns`, resetForm);
                        if (response.success) {
                                        ...
                                        <Row className='flex flex-row items-start justify-between w-[100%]'>
                                            ...
                                            <Col className='w-[20%]'>
                                                <SelectField
                                                    ...
                                                    keyName={`return_entries[${index}].sales_entry`}
                                                    options={sale.sales_entries.map((entry) => ({ value: entry.id, label: entry.stock_name }))}
                                                />
                  ...
                  <Col className='w-[30%]'>
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (values.return_entries.length < sale.sales_entries.length) {
