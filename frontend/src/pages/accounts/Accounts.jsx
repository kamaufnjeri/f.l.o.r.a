    ...
    <div className='flex-1 flex flex-col items-center justify-center relative h-full mr-2'>
      <div className='flex flex-row w-full items-center justify-between'>
        <form onSubmit={handleSubmit} className='flex h-10 flex-row self-start w-[40%] border-2 border-gray-800 rounded-md text-black relative'>
          ...
          {accounts?.accounts?.length > 0 && searchItem.name && <div className='max-h-36 overflow-auto  custom-scrollbar absolute left-0 top-10 flex flex-col bg-gray-800 p-2 rounded-md w-full z-10 text-white'>
            {accounts?.accounts?.map((account) => (<Link to={`${account.id}`} className='hover:bg-white hover:text-gray-800 w-full cursor-pointer rounded-md p-1'>{account.name}</Link>))}
          </div>}
        </form>
      ...
      <div className='overflow-auto custom-scrollbar flex flex-col flex-1 h-full w-full m-2'>
        <div className='w-full flex flex-row text-xl font-bold border-y-2 border-gray-800 border-l-2'>
          ...
          <span className='w-[20%] border-gray-800 border-r-2 p-1'>Balance  ({currentOrg.currency})</span>
        ...
        {accountsData?.results?.data?.accounts && accountsData.results.data.accounts.map((account, index) => (
          <Link to={`${account.id}`} className='w-full flex flex-row text-bold border-b-2 border-gray-800 border-l-2 hover:bg-gray-300 hover:cursor-pointer' key={account.id}>
            ...
            <span className='w-[20%] border-gray-800 border-r-2 p-1 text-right'>{account.account_balance}</span>
          </Link>
        ))}
        {accountsData?.results?.data?.totals &&
          <span className='w-full flex flex-row text-bold border-b-2 border-gray-800 border-l-2 font-bold underline text-right'>
            ...
            <span className='w-[20%] border-gray-800 border-r-2 p-1'>{accountsData?.results?.data?.totals?.balance}</span>
